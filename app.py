...
def get_backtest_engine_class():
    ...
    if _BACKTEST_ENGINE_CLASS is None:
        try:
            from backtest import BacktestEngine as backtest_engine_class

            _BACKTEST_ENGINE_CLASS = backtest_engine_class
        except ImportError:
            _BACKTEST_ENGINE_CLASS = _UnavailableBacktestEngine
...
def start_trader_bot_process():
    current_state = get_trader_bot_process_state()
    if current_state["running"]:
        return True, f"Bot já está ativo (PID {current_state['pid']})."
    ...
    return True, f"Bot trader iniciado em background (PID {process.pid})."
...
def render_trader_bot_runtime_controls(section_key: str = "bot_trader_runtime"):
    ...
    with bot_control_col1:
        if st.button("▶️ Ligar Bot Trader", key=f"{section_key}_start"):
            success, message = start_trader_bot_process()
            if success:
                st.success(message)
                st.rerun()
            else:
...
def main():
    ...
    if 'telegram_trading_bot_started' not in st.session_state:
        ...
        st.session_state.telegram_trading_bot_started = False
    if 'trader_bot_pid' not in st.session_state:
        st.session_state.trader_bot_pid = None
    ...
    if 'backtest_robustness_results' not in st.session_state:
    ...
    if active_dashboard_section == "backtest":
        ...
        with config_tab3:
            ...
            enable_global_robustness = st.checkbox(
                ...
                value=False,
                help="Roda o setup atual em uma cesta oficial multi-mercado e múltiplos horizontes temporais.",
        ...
        with col4:
            if enable_global_robustness and st.button(
                "🧱 Matriz Global",
                disabled=not robustness_ready or not allowed_execution_setups,
                help="Valida o setup atual em uma cesta oficial multi-mercado e multi-horizonte.",
                width="stretch",
